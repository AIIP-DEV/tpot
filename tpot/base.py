...
class TPOTBase(BaseEstimator):
    ...
    def _check_periodic_pipeline(self, gen):
        ...
        if self.custom_checkpoint and callable(self.custom_checkpoint):
            self.custom_checkpoint.set_generation(gen)
        ...
        try:
            ...
            if self._n_jobs == 1 and not self.use_dask:
                for ind_str, sklearn_pipeline, ind in zip(eval_individuals_str, sklearn_pipeline_list, individuals):
                    ...
                    self.custom_checkpoint.update_pipeline_score(self, ind_str, val, sklearn_pipeline, ind)
    ...
    def _update_evaluated_individuals_(self, result_score_list, eval_individuals_str, operator_counts, stats_dicts):
        ...
        for result_score, individual_str in zip(result_score_list, eval_individuals_str):
            if type(result_score) in [float, np.float64, np.float32]:
                ...
                if self.custom_checkpoint and callable(self.custom_checkpoint):
                    self.custom_checkpoint.update_evaluation(
                        self,
